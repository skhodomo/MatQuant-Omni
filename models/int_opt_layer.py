...
class QuantOPTDecoderLayer(nn.Module):
    def __init__(
        ...
        # 속성 초기화
        self.let = getattr(args, 'let', False)
        self.qkv_smooth_scale = nn.Parameter(torch.ones(1)) if self.let else None
        self.qkv_smooth_shift = nn.Parameter(torch.zeros(1)) if self.let else None
        self.fc1_smooth_scale = nn.Parameter(torch.ones(1)) if self.let else None
        self.fc1_smooth_shift = nn.Parameter(torch.zeros(1)) if self.let else None
    ...
    def smooth_and_quant_inplace(self):
        ...
        if self.let:
            ...
            smooth_ln_fcs_inplace(self.self_attn.v_proj,self.self_attn.out_proj,
    ...
    def smooth_and_quant_temporary(self):
        if self.let:
            ...
            smooth_ln_fcs_temporary(self.final_layer_norm,[self.fc1],
                                    self.fc1_smooth_scale,self.fc1_smooth_shift)
            smooth_ln_fcs_temporary(self.self_attn.v_proj,self.self_attn.out_proj,
                                self.out_smooth_scale, self.out_smooth_shift)
